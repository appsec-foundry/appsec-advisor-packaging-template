...
print_sharing_step() {
  ...
  echo "     Try the rollout locally first:"
  echo "       make install-local"
  echo "       Registers build/ as a throwaway local marketplace; your organization's"
  echo "       shared Marketplace is not touched."
  echo ""
  echo "     Option 1 - Tagged release, built by CI:"
  echo "       a) make ci-github        # or: make ci-gitlab - then commit the CI file"
  echo "       b) make release RELEASE_VERSION=${PACKAGE_VERSION}"
  echo "          Requires a clean main that matches origin/main. CI rebuilds the plugin"
  echo "          and publishes ${PLUGIN_NAME}-${PACKAGE_VERSION}.zip, .tgz and .sha256 as release assets."
  echo "       c) Developers load the ZIP asset with:"
  echo "            claude --plugin-url \"<direct HTTPS URL to ${PLUGIN_NAME}-${PACKAGE_VERSION}.zip>\""
  echo ""
  echo "     Option 2 - Manual archive hosting, without CI:"
  echo "       a) make release-package"
  echo "       b) Upload dist/${PLUGIN_NAME}-${PACKAGE_VERSION}.zip and its .sha256 file to an"
  echo "          approved internal HTTPS location."
  echo "       c) Developers load the ZIP the same way as in Option 1 c)."
  echo ""
  echo "     Option 3 - Internal Marketplace:"
  echo "       a) Commit the built plugin to a separate marketplace repository, alongside"
  echo "          a .claude-plugin/marketplace.json entry for it. Keep build/ out of this"
  echo "          packaging repository; it is generated output."
  echo "       b) Developers register the marketplace and install from it:"
  echo "            claude plugin marketplace add <marketplace-git-url>"
  echo "            claude plugin install ${PLUGIN_NAME}@<marketplace-name>"
  echo ""
...
echo "       - org-profile/org-profile.yaml"
echo "           identity, requirements, presets, banner, baseline, policy,"
echo "           guardrails, skill toggles, hooks, MCP servers"
echo "       - org-profile/context/organization.md"
echo "           replace the example with your organization context"
echo "       - org-profile/package-policy.yaml"
echo "           allowlist of packaged skills, hooks, and MCP servers"
echo "       - org-profile/actors/custom-actors.yaml"
echo "           organization-specific threat actors"
echo "       - org-profile/hooks/*.py"
echo "           scripts for the hooks declared in org-profile.yaml"
echo "       - org-skills/<skill-id>/SKILL.md"
echo "           organization-owned skills packaged next to upstream skills"
echo "       - Makefile"
echo "           PACKAGE_VERSION and INTERNAL_REPOSITORY_URL"
